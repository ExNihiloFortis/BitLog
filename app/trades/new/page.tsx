...
export default function NewTradePage() {
  ...
  // ====== Campos base ======
  const [dtOpen, setDtOpen] = useState<string>(() =>
    DateTime.now().setZone(MAZ_TZ).toISO({ suppressMilliseconds:true }) || ''
  )
  ...
  .from('ea_presets')
  .insert([{ name, user_id: userId }])   // ← CLAVE
  .select('id,name')
  .single()
...
if (!SIDE_ALLOWED.includes(sideNormRaw as any)) {
  alert('Selecciona LADO: BUY o SELL (obligatorio).')
  return
}

// La BD aún espera LONG/SHORT por el CHECK existente
...
if (!Number.isFinite(entryNum)) {
  alert('Falta el precio de ENTRADA (obligatorio).')
  return
}


const eaSideNorm = (() => {
  const v = (eaSide || '').trim().toUpperCase()
  if (v === '') return null
  if (!SIDE_ALLOWED.includes(v as any)) {
    alert('Lado (EA) inválido. Solo BUY o SELL.')
    return null
  }
  return v.toLowerCase() as 'buy'|'sell'
})()

  // --- Normalizaciones EA (usar MAYÚSCULAS y exigir side cuando hay EA) ---
const hasEA = !!(ea || '').trim();
...
if (hasEA) {
  ...
  if (!EA_SIDE_ALLOWED.includes(raw as any)) {
    alert('Si capturas un EA, debes elegir Lado (EA): BUY o SELL.');
    return; // detenemos el submit para no violar el CHECK
  }
...
const eaTimeframeDb = hasEA ? (eaTimeframe || null) : null;
      ...
      const ins = await supabase.from('trades').insert([insertPayload]).select('id,user_id').single()
      if (ins.error || !ins.data) { alert(ins.error?.message || 'No se pudo guardar'); return }
      const newId = ins.data.id as number
      const uid = ins.data.user_id as string
      ...
      if (stagedPics.length) {
        ...
        for (const f of stagedPics) {
          try {
            ...
            const path = `u_${uid}/t_${newId}/${Date.now()}_${safe.replace(/\.(png|webp)$/i,'.jpg')}`
            ...
            await supabase.from('attachments').insert([{ trade_id: newId, path, sort_index: order++ }])
          } catch (e:any) {
      ...
      router.replace(`/trades/${newId}`)
    } finally {
      setLoading(false)
    }
  }, [
    userId, dtOpen, dtClose, symbol, side, entry, sl, tp, size, commission, swap, pips, pnl, rTarget,
    trend, pattern, session, emotion, durationMin, tag, closeReason, broker, brokerTradeId, platform,
    ea, eaTimeframe, eaSide, eaQuality, eaSL, eaTP, eaNote, stagedPics, router
  ])
  ...
  return (
    <div className="space-y-4" onPaste={onPaste}>
      <h1 className="text-xl font-semibold">Nuevo Trade (completo)</h1>
  ...
  >
    ...
    <option>SELL</option>
      ...
      <div className="flex gap-2">
        ...
        <Link href="/trades" className="px-4 py-2 bg-zinc-800 rounded text-sm border border-zinc-700">Cancelar</Link>
      </div>
    </div>
  )
}
