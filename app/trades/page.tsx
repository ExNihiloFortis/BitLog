...
export default function Trades() {
  ...
  const loadDistincts = async (uid: string) => {
    const { data: sym } = await supabase
      .from('trades').select('symbol').eq('user_id', uid)
      .not('symbol', 'is', null).order('symbol', { ascending: true }).range(0, 4999)
    const symbols = Array.from(new Set((sym||[]).map(r => r.symbol as string))).filter(Boolean)

    const { data: easData } = await supabase
      .from('trades').select('ea').eq('user_id', uid)
      .not('ea', 'is', null).order('ea', { ascending: true }).range(0, 4999)
  ...
  // carga página con keyset pagination
  const loadPage = async (reset=false) => {
    ...
    try {
      ...
      if (!reset && cursor) {
        const orExpr = `dt_utc.lt.${cursor.dt},and(dt_utc.eq.${cursor.dt},id.lt.${cursor.id})`
      ...
      if (reset) setRows(batch); else setRows(prev => [...prev, ...batch])
  ...
  const displayRows = useMemo(() => {
    ...
    arr.sort((a,b) => {
      ...
      if (sortBy === 'pnl') { va = (va ?? 0); vb = (vb ?? 0); return (va - vb) * dir }
      if (sortBy === 'id')  { return ((a.id) - (b.id)) * dir }
      if (sortBy === 'dt_utc') return (va < vb ? -1 : va > vb ? 1 : 0) * dir
  ...
  // CSV seguro: comillas cuando hay comas, comillas o saltos de línea
  ...
  const exportCSV = () => {
    ...
    const lines = displayRows.map(r=>{
      const fecha = localFmt(r.dt_utc)          // contiene coma
      return [
        r.id,
        csvEscape(fecha),                       // ← entre comillas
        csvEscape(r.symbol),
        csvEscape(r.side),
        (r.pnl ?? 0),
        csvEscape(r.ea ?? ''),
